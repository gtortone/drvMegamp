...
void mtimeSortList(struct ELLLIST *l);
...
STATIC void mtimeSortList(ELLLIST *l) {

   int i,j;
   struct configFileListItem *p, *q;
   int n = ellCount(l);
   char *buf;
   time_t t;

   p = (struct configFileListItem *) ellFirst(l);

   for(i=0; i<n-1; i++) {

      q = (struct configFileListItem *) ellNext(&(p->node));
      ...
      for(j=i+1; j<n; j++) {
         ...
         q = (struct configFileListItem *) ellNext(&(q->node));
      }

      p = (struct configFileListItem *) ellNext(&(p->node));
   }
}
	...
	if (pdir) {
		...
		while ((pdirent=readdir(pdir))) {
			...
			if (strncmp(config_underscore, pdirent->d_name, strlen(config_underscore)) == 0) {
				...
				if (pchar) {
					*pchar = '\0';
					...
					makeNfsPath(fullpath, saveRestoreFilePath, filename);
                                        // fetch configuration file mtime
                                        stat(fullpath, &st);
                                        pLI->mtime = st.st_mtime;
                                        if (save_restoreDebug) printf("findConfigFiles: mtime %ld\n", (long)st.st_mtime);
                ...
                // sort list by modification time (mtime)
                mtimeSortList(configMenuList);

		if (save_restoreDebug) {
			pLI = (struct configFileListItem *) ellFirst(configMenuList);
			printf("findConfigFiles: \n");
			while (pLI) {
				printf("	name='%s'; desc='%s'; mtime=%ld\n", pLI->name, pLI->description, (long)pLI->mtime);
